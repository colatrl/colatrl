...
// This header file is shared by eBPF kernel programs (C, 64-bit BPF
// architecture) and userspace (which could potentially be 32-bit C++/Java
// x86/arm/...)
//
...
#ifdef __cplusplus
#define ERR(x) #x,
static const char *bpf_clat_errors[] = {BPF_CLAT_ERRORS};
#undef ERR
#endif

#define ERR(x) BPF_CLAT_ERR_##x,
typedef enum {
  BPF_CLAT_ERRORS JUST_MAKE_IT_32_BIT = 0xFFFFFFFF,
} ClatErrorKey;
#undef ERR
STRUCT_SIZE(ClatErrorKey, 4);

#define TC_COUNT3(cnttype, dirlong, counter, dirshort)                         \
  do {                                                                         \
...
#define TC_COUNT2(cnttype, dirlong, counter, dirshort)                         \
  TC_COUNT3(cnttype, dirlong, counter, dirshort)

// Before using these, DIRLONG and DIRSHORT should be #defined to INGRESS/EGRESS
// and in/out
#define TC_COUNT(counter) TC_COUNT2(CNT, DIRLONG, counter, DIRSHORT)
#define TC_PUNT(counter)                                                       \
  do {                                                                         \
    TC_COUNT2(IGN, DIRLONG, counter, DIRSHORT);                                \
    return TC_ACT_PIPE;                                                        \
  } while (0)
#define TC_DROP(counter)                                                       \
  do {                                                                         \
    TC_COUNT2(ERR, DIRLONG, counter, DIRSHORT);                                \
    return TC_ACT_SHOT;                                                        \
  } while (0)
...
typedef struct {
  ...
  std::string toString() const {
    char buf[6 * 3];
    snprintf(buf, sizeof(buf), "%02x:%02x:%02x:%02x:%02x:%02x", mac8[0],
             mac8[1], mac8[2], mac8[3], mac8[4], mac8[5]);
    return buf;
  }
#endif
} MacAddress;
...
typedef struct {
  ...
  std::string toString() const {
    return "{ifindex: " + ifindex_to_string(ifindex) +
...
typedef struct {
  ...
  std::string toString() const {
    return "{pfx96: " + ip6_to_string(pfx96) +
           "/96, local4: " + ip4_to_string(local4) + "}";
  }
#endif
} ClatSrcIpValue;
...
typedef struct {
  ...
  struct in6_addr
      pfx96; // The source /96 nat64 prefix, bottom 32 bits must be 0
  struct in6_addr local6; // The full 128-bits of the destination IPv6 address

#ifdef __cplusplus
  static const char *name() { return "ClatIngress6Key"; }
  std::string toString() const {
    return "{iif: " + ifindex_to_string(iif) +
           ", pfx96: " + ip6_to_string(pfx96) +
           "/96, local6: " + ip6_to_string(local6) + "}";
  }
#endif
} ClatIngress6Key;
...
typedef struct {
  ...
  struct in6_addr
      pfx96; // The destination /96 nat64 prefix, bottom 32 bits must be 0
  ...
  std::string toString() const {
    ...
    return "{oif: " + ifindex_to_string(oif) +
           ...
           ", oifIsEthernet: " + (oifIsEthernet ? "true" : "false") + padStr +
...
typedef struct {
  ...
  std::string toString() const {
    std::string padStr = pad ? ", pad: " + std::to_string(pad) + "!" : "";
    return "{count: " + std::to_string(count) + padStr +
           ", last_time: " + std::to_string(last_time) + "}";
  }
#endif
} ClatTimerValue;

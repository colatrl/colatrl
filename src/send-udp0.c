...
void ptr_u8_freer(uint8_t *const *mem) { free(*mem); }
...
int main(const int argc, const char *const argv[]) {
  if (argc != 4) {
    fprintf(stderr,
            "Usage: %s <destination_ipv4> <destination_port> <payload_size>\n",
            argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  defer(fd_closer) const int sockfd =
  ...
  // 12 bytes (which is less than 20 bytes of ipv4 header) + 8 + max 65507 =
  // 65527, so no overflow

  uint16_t pseudogram_size =
      sizeof(struct pseudo_header) + sizeof(struct udphdr) + payload_size;
  defer(ptr_u8_freer) uint8_t *pseudogram = calloc(pseudogram_size, 1);
  if (!pseudogram) {
  ...
  *uh = (struct udphdr){
  ...
  // The kernel will ignore the checksum if SO_NO_CHECK is set, but we still
  // calculate and print it.
  int disable_checksum = 1;
  // again:
  if (setsockopt(sockfd, SOL_SOCKET, SO_NO_CHECK, &disable_checksum,
                 sizeof(disable_checksum)) < 0) {
    perror("setsockopt(SO_NO_CHECK) failed");
    exit(EXIT_FAILURE);
  }

  ssize_t sent_bytes = sendto(sockfd, payload, payload_size, 0,
  ...
  printf("Sent a %zd-byte UDP packet to %s:%d (%08X:%d -> %08X:%d) with no "
         "checksum (expected 0x%04X)\n",
         sent_bytes, dest_ip_str, dest_port, ntohl(source_addr.sin_addr.s_addr),
         ntohs(source_addr.sin_port), ntohl(dest_addr.sin_addr.s_addr),
         ntohs(dest_addr.sin_port), correct_checksum);
